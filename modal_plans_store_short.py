# modal_plans_store_short.py
# Modal "plans" microservice (SHORT STRATEGY VERSION).
# Stores and serves investment plans created by your "Short" inference app.
#
# - Uses the SAME Modal Volume ("plans-cache") but a DIFFERENT directory (/vol/plans_short).
# - Keeps your Short/Bear plans completely isolated from your Long/Bull plans.
#
# Deploy once:
#   modal deploy modal_plans_store_short.py
#
# Then call from Streamlit by looking up "inmemory-latest-plans-short".
...
# --- CONFIGURATION CHANGES FOR SHORT STRATEGY ---
APP_NAME = "inmemory-latest-plans-short"
PLANS_DIR = "/vol/plans_short"             # Separate directory for short plans
PLANS_INDEX = f"{PLANS_DIR}/plans.jsonl"   # Separate index file
...
# We use the same physical volume, but we will write to a different folder inside it.
...
def _ensure_dir():
    # Ensures /vol/plans_short exists
...
# Note: Mounting to /vol/plans_short via the global variable PLANS_VOL
@app.function(image=base_image, volumes={"/vol/plans_short": PLANS_VOL}, timeout=60*5)
def save_plan(plan: Dict[str, Any]) -> Dict[str, Any]:
    """
    Enriches a fresh plan (from inference) with buy prices and share quantities,
    then persists it on the 'plans-cache' volume in the SHORT directory.
...
def list_plans() -> List[Dict[str, Any]]:
    ...
    Reads from the SHORT directory.
    """
    items = _load_index()
