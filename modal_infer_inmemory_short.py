# modal_infer_inmemory_short.py
# Modalized "latest-day" inference (SHORT / BEAR STRATEGY):
# - CPU function builds texts in memory (no files)
# - GPU function loads your ckpt from a Modal Volume and scores items
# - Prints a top-k allocation table for the LEAST promising stocks.
#
# Usage:
#   modal deploy modal_infer_inmemory_short.py
...
# -----------------------------
# NAME CHANGED HERE TO PREVENT OVERWRITING EXISTING APP
app = modal.App("inmemory-latest-infer-short")
...
# Persistent volumes (Reuse existing volumes)
...
def run_infer_remote(
    ...
    # ------------------------------------------------------------------
    #  SHORT STRATEGY:
    #  1. Softmax on negative scores => worst scores get highest weight
    #  2. Sort ascending => worst scores appear first
    # ------------------------------------------------------------------
...
def main(
    ...
    print(f"\n=== Inference — DATE={result['date']} — top-{result['k']} LEAST PROMISING (SHORT) (temp={result['temp']:.3f}, invest=${result['invest_amt']:.2f}) ===")
